...
from utils.attack import (
    ...
    copy_paste_attack,
    scramble_attack, word_deletion_attack, word_insertion_attack, synonym_replacement_attack,
)
...
def main(args):
    ...
    # delete
    ##########################################################################
    elif args.attack_method == "delete":
        print("Running DELETE attack")
        # If no cp_attack_min_len specified, use args.max_new_tokens
        if args.cp_attack_min_len == 0:
            args.cp_attack_min_len = args.max_new_tokens
        tokenizer = load_tokenizer(args)
        word_deletion_attack_partial = partial(
            word_deletion_attack,
            tokenizer=tokenizer,
            args=args,
        )
        gen_table_attacked_ds = gen_table_ds.map(word_deletion_attack_partial, batched=False)
    ###########################################################################
    # insert
    ###########################################################################
    elif args.attack_method == "insert":
        print("Running INSERT attack")
        # If no cp_attack_min_len specified, use args.max_new_tokens
        if args.cp_attack_min_len == 0:
            args.cp_attack_min_len = args.max_new_tokens
        tokenizer = load_tokenizer(args)
        word_insertion_attack_partial = partial(
            word_insertion_attack,
            tokenizer=tokenizer,
            args=args,
        )
        gen_table_attacked_ds = gen_table_ds.map(word_insertion_attack_partial, batched=False)
    ###########################################################################
    # synonym
    ###########################################################################
    elif args.attack_method == "synonym":
        print("Running SYNONYM attack")
        # If no cp_attack_min_len specified, use args.max_new_tokens
        if args.cp_attack_min_len == 0:
            args.cp_attack_min_len = args.max_new_tokens
        tokenizer = load_tokenizer(args)
        synonym_replacement_attack_partial = partial(
            synonym_replacement_attack,
            tokenizer=tokenizer,
            args=args,
        )
        gen_table_attacked_ds = gen_table_ds.map(synonym_replacement_attack_partial, batched=False)
    ###########################################################################
    # Copy-paste attack
    ###########################################################################
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--run_name",
        type=str,
        default="4b-100T-4R-lefthash_copy-paste-cp=.1attack",
        help="The unique name for the run.",
    )
    parser.add_argument(
        "--input_dir",
        type=str,
        default="./experiments/sample-run/4b-100T-4R-lefthash",
        help="The directory containing the input files.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./experiments/sample-run/4b-100T-4R-lefthash_copy-paste-cp=.1attack",
        help=(
