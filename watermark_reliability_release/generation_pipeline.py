...
# from partition_watermark_processor import WatermarkLogitsProcessor
...
from utils.block_generation import (
    MAX_GENERATIONS,
    load_model,
    load_hf_dataset,
    check_input_lengths,
...
def main(args):
    ...
    if args.method=="Redblock-bh":
        ...
        watermark_processor = WatermarkLogitsProcessor(
            ...
            token_to_position_mapping=bh_mapping,
            **wm_kwargs
        )
    ...
    # 汇总延迟数据
    wm_encoding_times = []
    non_wm_encoding_times = []

    for example in processed_examples:
        wm_encoding_times.append(example["wm_encoding_time"])
        non_wm_encoding_times.append(example["non_wm_encoding_time"])

    # 计算统计指标
    wm_encoding_mean = np.mean(wm_encoding_times)
    wm_encoding_std = np.std(wm_encoding_times)
    non_wm_encoding_mean = np.mean(non_wm_encoding_times)
    non_wm_encoding_std = np.std(non_wm_encoding_times)

    print(f"Watermark Encoding Time: mean={wm_encoding_mean:.6f} s, std={wm_encoding_std:.6f} s")
    print(f"Non-Watermark Encoding Time: mean={non_wm_encoding_mean:.6f} s, std={non_wm_encoding_std:.6f} s")
    #################################################################################################################
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="Redblock",
        help="Method of watermarked huggingface LM generation pipeline",
    )
    parser.add_argument(
        "--bh_map_load_path",
        type=str,
