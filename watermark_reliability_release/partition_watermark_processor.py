...
class WatermarkBase:
    def __init__(
        ...
        **kwargs
        ...
        self._initialize_semantic_partition()
    ...
    def _initialize_semantic_partition(self):
        vocab_embeddings, _ = self.get_vocab_embeddings()
        cluster_labels = self.cluster_vocab(vocab_embeddings)
        sorted_indices = np.argsort(cluster_labels)
        sorted_vocab = torch.tensor([self.vocab[i] for i in sorted_indices], device=self.device)
        self.rng = torch.Generator() if self.rng is None else self.rng
        self.rng.manual_seed(54321)
        self.sorted_vocab_permutation = torch.randperm(len(sorted_vocab), generator=self.rng)
        self.sorted_vocab = sorted_vocab[self.sorted_vocab_permutation]
    ...
    def get_vocab_embeddings(self):
        # 加载预训练的 Word2Vec 模型（需提前下载）
        word_vectors = KeyedVectors.load_word2vec_format('../../gensim-data/word2vec-google-news-300/word2vec-google-news-300', binary=True)

        # 将 token ID 转换为词
        vocab_words = [self.tokenizer.decode([token_id]) for token_id in self.vocab]
        vocab_embeddings = []

        for word in vocab_words:
            if word in word_vectors:
                vocab_embeddings.append(word_vectors[word])
            else:
                # 对于不在模型中的词，使用随机向量
                vocab_embeddings.append(np.random.rand(word_vectors.vector_size))

        return np.array(vocab_embeddings), vocab_words

    def cluster_vocab(self, vocab_embeddings):
        # 颜色列表数量由 1/self.gamma 确定
        k = int(np.floor(1 / self.gamma))  # 例如，gamma=0.5 时，k=2
        kmeans = KMeans(n_clusters=k, random_state=54321).fit(vocab_embeddings)
        cluster_labels = kmeans.labels_
        return cluster_labels

    def _get_greenlist_ids(self, input_ids: torch.LongTensor) -> torch.LongTensor:
        # 在初始化时预计算聚类（只需执行一次）
        if not hasattr(self, 'sorted_vocab_permutation'):
            vocab_embeddings, vocab_words = self.get_vocab_embeddings()
            cluster_labels = self.cluster_vocab(vocab_embeddings)
        ...
        candidate_greenlist = torch.chunk(self.sorted_vocab, int(np.floor(1 / self.gamma)))
        self._seed_rng(input_ids)  # 更新 self.message_char
    ...
    def _get_colorlist_ids(self, input_ids: torch.LongTensor) -> torch.LongTensor:
        ...
        colorlist = torch.chunk(self.sorted_vocab, int(np.floor(1 / self.gamma)))
        self._seed_rng(input_ids)  # 更新 self.message_char
        return colorlist
...
class WatermarkLogitsProcessor(WatermarkBase, LogitsProcessor):
    ...
    def _bias_greenlist_logits(
        ...
        if denylist_flag:
            ...
            # scores[colorlist_mask] = scores[colorlist_mask] * greenlist_bias
            scores[colorlist_mask] = scores[colorlist_mask] * 1.15
        return scores
