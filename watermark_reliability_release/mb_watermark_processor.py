...
class WatermarkBase:
    def __init__(
        ...
        device: str = "cuda",
        token_to_position_mapping: list[int] = None,
        **kwargs
    ):
        ...
        print("self.converted_msg_length",self.converted_msg_length)
        ###############################################################################
        # self.code_length = code_length  # 编码后的码字长度
        # self.rs = galois.ReedSolomon(n=self.code_length, k=self.original_msg_length)
        #################################################################################
        ...
        self.token_to_position_mapping = token_to_position_mapping
    ...
    def _seed_rng(self, input_ids: torch.LongTensor) -> None:
        ...
        elif self.token_to_position_mapping is not None:
            if input_ids.shape[-1] < 1:
                raise ValueError("Need at least one previous token to determine position")
            previous_token = input_ids[-1].item()
            position_index = self.token_to_position_mapping[previous_token]
            self.bit_position = position_index + 1  # Convert 0-based index to 1-based position
        else:
        ...
        # print(f"message_char: {self.message_char}")
        # enable for long, interesting streams of pseudorandom numbers: print(prf_key)
        self.rng.manual_seed(prf_key % (2**64 - 1))
        # self.rng.manual_seed(54321)  # safeguard against overflow from long
        # print(f"rng.manual_seed: {self.rng.manual_seed(54321)}")
    ...
    def set_message(self, binary_msg: str = ""):
        ...
        # ################################################################
        # msg_int= int(binary_msg, 2)
        # codeword = self.rs.encode([msg_int])
        # self.encode_message = bin(codeword)[2:].zfill(self.code_length)
        # self.converted_message = self._convert_binary_to_base(self.encode_message)
        # ################################################################
    ...
    def _convert_binary_to_base(self, binary_msg: str):
        ...
        # print("converted_msg_length", self.converted_msg_length)
...
class WatermarkLogitsProcessor(WatermarkBase, LogitsProcessor):
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        feedback_bias = self.feedback_args.get("feedback_bias", 0.5)
...
class WatermarkDetector(WatermarkBase):
    ...
    * z_threshold -> Changing this threshold will change the sensitivity of the detector.
    ...
    def dummy_detect(
        ...
        score_dict.update(dict(cand_acc_16=float("nan")))
        ...
        score_dict.update(dict(cand_acc_ablation_16=float("nan")))
        ...
        if return_z_score:
            ...
            score_dict.update(dict(z_score_per_position=[]))
        if return_p_value:
    ...
    def _score_sequence(
        ...
        **kwargs,
        ...
        if True:
            ...
            # 其中green_cnt_by_position为一个字典形式模拟计数矩阵。
            # frequencies_table表示
            # ngram to watermark lookup表示
            # ngram to position lookup表示
        else:
            ngram_to_watermark_lookup, frequencies_table, ngram_to_position_lookup, green_cnt_by_position \
        ...
        for p in range(1, self.converted_msg_length + 1):
            ...
            total_count = position_cnt.get(p, 0)
            z_score_p = self._compute_z_score(green_cnt, total_count)
            z_score_per_position.append(z_score_p)
        ...
        # 使用 Reed-solomon 解码
        # pred_msg_str = "".join(map(str, best_prediction))
        # try:
        #     pred_decimal = int(pred_msg_str, self.base)
        #     pred_binary = bin(pred_decimal)[2:].zfill(self.code_length)
        #     decoded_msg = self.rs.decode([int(pred_binary, 2)])
        #     recovered_message = bin(decoded_msg[0])[2:].zfill(self.original_msg_length)
        # except Exception as e:
        #     print(f"解码失败: {e}")
        #     recovered_message = "0" * self.original_msg_length
        ...
        if return_bit_match:
            ...
            score_dict.update(dict(cand_acc_16=max(prediction_results['confidence'][:17]) / total_bits))
            ...
            score_dict.update(dict(cand_acc_ablation_16=max(prediction_results['random'][:17]) / total_bits))
        ...
        if return_z_score:
            ...
            score_dict.update(dict(z_score_per_position=z_score_per_position))
        if return_p_value:
            z_score = score_dict.get("z_score")
            if z_score is None:
                z_score = self._compute_z_score(green_token_count, num_tokens_scored)
    ...
    def _predict_message(self, position_cnt, green_cnt_by_position, p_val_per_pos,
        ...
        for pos in range(1, self.converted_msg_length + 1):
            ...
            if position_cnt.get(pos) is None: # no allocated tokens (may happen when T / b is small)
                ...
                sorted_idx = np.argsort(green_counts)       # 使用green_count的大小关系对idx进行排序
                max_idx, next_idx = sorted_idx[-1], sorted_idx[-2]      #green_counts第一第二大的标签
                confidence_per_pos.append((-p_val, max_idx, next_idx, pos))     # 记录每个位置的p值大小，最大值所在的标签以及次大值所在的标签
        ...
        confidence_per_pos = sorted(confidence_per_pos, key=lambda x: x[0])[:num_candidate_position]    # 将每个位置的confidence信息根据p值排序
        msg_prediction_list = [msg_prediction]
        cnt = 0
        candidate_iter = iter(powerset(confidence_per_pos))
        while cnt < num_candidates:     #如果选出的候选信息数量不够
            try:
    ...
    def _compute_ber(self, pred_msg: list, message: str):
        ...
        # if use_ecc:     # RS ECC
        #     from pyfinite import ffield, rs_code
        #     n = 4  # 码字长度
        #     k = 2  # 消息长度（符号数）
        #     t = 1  # 纠错能力
        #     F = ffield.FField(4)  # GF(16)
        #     rs = rs_code.RSCode(F, n, k)
        #     pred_bytes = [int(binary_pred[i:i + 4], 2) for i in range(0, len(binary_pred), 4)]
        #     decoded = rs.Decode(pred_bytes)
        #     if decoded:
        #         binary_pred = ''.join(format(x, '04b') for x in decoded)
        #     else:
        #         binary_pred = binary_pred[:self.original_msg_length]
