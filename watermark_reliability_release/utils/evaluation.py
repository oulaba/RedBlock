...
# from partition_watermark_processor import WatermarkDetector
...
def load_detector(args):
    ...
    if args.method == "Redblock-bh":
        if os.path.exists(args.bh_map_load_path):
            with open(args.bh_map_load_path, "rb") as f:
                bh_mapping = pickle.load(f)
        else:
            raise FileNotFoundError(f"Balanced hash mapping file not found: {args.bh_map_load_path}")
...
def compute_z_score(
    ...
    if input_text == "":
        ...
        if debug:
            ...
            # _, _, _, green_cnt_by_position, _ = watermark_detector._score_ngrams_in_passage_sequential(input_ids)
            # matrix = np.array([green_cnt_by_position[i] for i in range(1, detector.converted_msg_length + 1)])
            # print("Green token count matrix (rows: positions, columns: color lists):")
            # print(matrix)
        ...
        detection_time = end_time - start_time      # 记录检测时间
    ...
    # score_dict[f"{text_column_name}_detection_time"] = detection_time
...
#         oracle_model = AutoModelForCausalLM.from_pretrained(oracle_model_name,device_map = "auto", load_in_8bit = True)
#
...
def load_oracle_model(args):
    ...
    if args.load_fp16:
        ...
        oracle_model = AutoModelForCausalLM.from_pretrained(oracle_model_name, load_in_8bit=True, device_map="auto")
    if "llama" in oracle_model_name:
        oracle_tokenizer = LlamaTokenizer.from_pretrained(oracle_model_name)
        oracle_model.config.pad_token_id = oracle_tokenizer.pad_token_id = 0  # unk
        oracle_model.config.bos_token_id = 1
