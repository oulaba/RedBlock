...
def main(args):
    ...
    # # 汇总检测延迟数据
    # detection_times = []
    # for example in gen_table_w_zscore_ds:
    #     for col_name in ZSCORE_TEXT_COLUMN_NAMES:
    #         if col_name in example:
    #             detection_time_key = f"{col_name}_detection_time"
    #             if detection_time_key in example:
    #                 detection_times.append(example[detection_time_key])
    #
    # # 计算统计指标
    # detection_mean = np.mean(detection_times)
    # detection_std = np.std(detection_times)
    # print(f"Detection Time: mean={detection_mean:.6f} s, std={detection_std:.6f} s")
    ###########################################################################
            ...
            for test_stat in test_stats:
                for attacked in [False,True]:
                    try:
                        ...
                        if args.plot_low_fpr_tpr:
                            plot_low_fpr_tpr(
                                results,
                                test_stat=test_stat,
                                plot_path=f"low_fpr_tpr_{test_stat}.png"
                            )
...
def _roc_metrics_for_wandb(
        ...
        remove_nan=True,
        save_plot=False,
        plot_path="32b_block_uni_mul_roc_curve.png",
        save_data=False,
        data_path="./roc_data_32b_bh.csv"
):
    """
    Calculate ROC metrics and optionally save plot/data for visualization.

    Args:
        gen_table_ds: Dataset containing test statistics.
        test_stat: Name of the test statistic (default: "z_score").
        prefix: Prefix for column names (default: "").
        attacked: Whether data is attacked (default: False).
        remove_nan: Drop rows with NaN values (default: True).
        save_plot: Save ROC curve as an image (default: True).
        plot_path: Path to save the ROC plot (default: "16b_block_uni_mul_roc_curve.png").
        save_data: Save ROC data to a file (default: False).
        data_path: Path to save ROC data (default: "roc_data.csv").

    Returns:
        roc_auc: Area under the ROC curve (scalar).
        fpr: False Positive Rate array.
        tpr: True Positive Rate array.
        thresholds: Thresholds corresponding to FPR and TPR.
        tpr_at_X_fpr: TPR at FPR < 1e-5 (or NaN if not applicable).
    """
    # Determine column names based on test_stat and attacked status
    ...
    # Drop NaNs in specified columns if requested
    if remove_nan:
    ...
    # Extract statistics and create labels
    ...
    print(f"detect name: {w_wm_col_name}")

    # Calculate TPR at FPR < 1e-5
    try:
        tpr_at_X_fpr = tpr[np.where(fpr < 1e-5)[0][-1]]
        print(f"tpr_at_X_fpr under 1e-5 : {tpr_at_X_fpr}")
    except IndexError:
        tpr_at_X_fpr = float("NaN")

    # Optionally save ROC plot
    if save_plot:
        plt.figure()
        ...
        plt.title(f'ROC Curve ({test_stat}{", attacked" if attacked else ", clean"})')
    ...
    # Optionally save ROC data to CSV
    if save_data:
        ...
        roc_data.to_csv(data_path, index=False)
        print(f"ROC data saved to {data_path}")

    return roc_auc, fpr, tpr, thresholds, tpr_at_X_fpr


def plot_low_fpr_tpr(results, methods, attacked=False, fpr_range=(1e-5, 1e-2), tpr_range=(0.6, 1.0),
                     plot_path="low_fpr_tpr_comparison.png"):
    plt.figure(figsize=(8, 6))

    # 样式设置：颜色和线型
    colors = ['#FFC107', '#00BCD4', '#3F51B5']  # 黄色、青色、深蓝色
    linestyles = ['-', '--', '-.']  # 实线、虚线、点线

    for idx, method in enumerate(methods):
        fpr = results[method]["fpr"]
        tpr = results[method]["tpr"]
        roc_auc = results[method]["roc_auc"]
        ...
        plt.plot(fpr_filtered, tpr_filtered,
    ...
    plt.xlabel('False Positive Rate (FPR)')
    plt.ylabel('True Positive Rate (TPR)')
    plt.title(f'Low FPR TPR Comparison ({"attacked" if attacked else "clean"})')
    plt.grid(True, which="both", ls="--", alpha=0.7)
    plt.legend()
    plt.savefig(plot_path)
    plt.close()
    print(f"Low FPR TPR plot saved to {plot_path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        default='../balance_hash/map_freq.pkl',
        help='[Used only in Redblock-BH] path to the saved balance hash mapping, should be a pickle file. When this option is set,' +
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Whether to plot low fpr TPR comparison.",
    )
    parser.add_argument(
        "--ppl_batch_size",
        type=int,
    ...
    parser.add_argument(
        "--input_dir",
        type=str,
        default="./experiments/block-uni-mul-run/8b-200T-4R-lefthash_copy-paste-cp=.1attack",
        help="The directory containing the input files.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./experiments/block-uni-mul-run/8b-200T-4R-lefthash_copy-paste-cp=.1attack_eval",
        help=(
